"""Console-script entry point for the lucy-notes-daemon package."""
...
TEMPLATE_STARTUP_ARGS: Template = [
    ...
    (
        ...
        str,
        None,
        "One or more directories to watch recursively. Example: --sys-notes-dirs ~/notes ~/work/notes",
    ),
    (
        "--sys-on-open-cooldown",
        int,
        20,
        "Cooldown for 'on_opened' events per file, in seconds. Prevents editor spam. Default: 20 seconds.",
    ),
]

DEFAULT_MODULES: List[AbstractModule] = [
    Banner(),
    Renamer(),
    TodoFormatter(),
    Today(),
...
def main() -> int:
    ...
    modules = ModuleManager(modules=DEFAULT_MODULES, args=unknown_args)
